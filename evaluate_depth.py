...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        dataloader = DataLoader(dataset, 128, shuffle=False, num_workers=opt.num_workers,
                                pin_memory=True, drop_last=False)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # encoder = networks.ResnetEncoder(opt.num_layers, False)
        encoder = networks.resnet_encoder.VAN_encoder(zero_layer_mlp_ratio=4, zero_layer_depths=1,  pretrained=False)
        # depth_decoder = networks.DepthDecoder(encoder.num_ch_enc)
        # depth_decoder = networks.depth_decoder.HRDepthDecoder(num_ch_enc=[64, 64, 128, 320, 512])
        depth_decoder = networks.depth_decoder.VAN_decoder(mlp_ratios=(4, 4, 4, 4), depths=(1, 1, 2, 1))
